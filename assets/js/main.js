...
function grax_tm_contact_form() {
    ...
    // Debería verse como: https://script.google.com/macros/s/AKfycbx.../exec
    const GOOGLE_SCRIPT_URL = 'https://script.google.com/macros/s/AKfycbzcMUOd6hJNYbsyGg5c0k4kq-y2jrUAsnUrBRW3-2bUkoK_LaBdfmu6wtqT8zD4GHj1/exec';
    ...
    if (contactForm) {
        contactForm.addEventListener('submit', function (e) {
            ...
            // Obtener los valores para validación básica
            ...
            // Recopilar todos los datos del formulario (incluyendo dinámicos)
            ...
            // Agregar ID del proyecto desde la URL
            const params = new URLSearchParams(window.location.search);
            const projectId = params.get("id");

            // Definir el campo 'proyecto' que coincide con la columna del Excel
            data.proyecto = "General"; // Valor por defecto

            if (projectId) {
                // Si tenemos ID, intentamos obtener el nombre bonito del título
                const titleElement = document.getElementById('hero-title');
                if (titleElement) {
                    data.proyecto = titleElement.textContent;
                } else {
                    data.proyecto = projectId;
                }
            } else {
                // Si estamos en el home (sin ID), verificamos si venía de un modal
                // (Opcional, pero útil si se quiere tracking más preciso en el futuro)
            }

            // Enviar a Google Sheets
            // Usamos mode: 'no-cors' y Content-Type: 'text/plain' para evitar problemas de CORS
            // Nota: Con 'no-cors' no podemos leer la respuesta, así que asumimos éxito si envía
            fetch(GOOGLE_SCRIPT_URL, {
                ...
                .catch(error => {
                    ...
                    alert('Hubo un error al enviar el mensaje. Intenta nuevamente o contáctanos por WhatsApp.');
                })
                .finally(() => {
